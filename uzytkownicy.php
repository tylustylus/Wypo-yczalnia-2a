...
<body>
    <div>
        ...
        <?php    
            switch ($typ) {
                case "":
                    $sql = "SELECT nr_prac, imie, nazwisko, pesel, login FROM pracownicy ORDER BY nazwisko, imie";
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        echo "    <input type='hidden' name='id' value='" . $row['nr_prac'] . "'>";
                ...
                case "nowy";
                    ...
                    <form action="uzytkownicy.php" method="GET">
                        <input type="hidden" name="typ" value="nowydb"><br>
                ...
                case "nowydb";
                    ...
                    if ($row = mysqli_fetch_array($result)){
                        echo "Podany login : $login istnieje już w systemie.";
                    } else {
                        $sql2 = "INSERT INTO pracownicy(pesel, imie, nazwisko, login, haslo) VALUES ('$imie','$nazwisko','$pesel','$login','$haslo')";
                        mysqli_query($connect, $sql2);
                        echo " Dodano nowego użytkownika:<br>
                        Imię: $imie<br>Nawisko:$nazwisko<br>Login:$login";
                        echo "<button onClick='history.back()'>Powrót</button>";
                    }
                    echo " Dodano nowego użytkownika NOWY DB" ;
                    break;
                case "edytuj";
                    $id = $_REQUEST['id'];
                    $sql = "SELECT imie , nazwisko , pesel , login FROM Pracownicy WHERE nr_prac = $id"; 
                    $result = mysqli_query($connect,$sql);
                    ...
                    <form action="uzytkownicy.php" method="GET">
                        <input type="hidden" name="typ" value="edytujdb"><br>
                ...
                case "edytujdb":
                    ...
                    $nazwisko = $_REQUEST['nazwisko'];
                    $login = $_REQUEST['login'];
                    $sql = "UPDATE pracownicy SET imie='$imie', nazwisko='$nazwisko', login='$login' WHERE nr_prac=$id";
                ...
                case "usun":
                    $id = $_REQUEST['id'];
                    $sql = "SELECT imie, nazwisko FROM pracownicy WHERE nr_prac = $id";
                    $result = mysqli_query($connect, $sql);
                    $row = mysqli_fetch_array($result);
                    ?>
                ...
                case "usundb":
                    ...
                    $imie = $_REQUEST['imie'];
                    $nazwisko = $_REQUEST['nazwisko'];
                    $sql = "DELETE FROM pracownicy WHERE nr_prac = $id";
                    mysqli_query($connect, $sql);
